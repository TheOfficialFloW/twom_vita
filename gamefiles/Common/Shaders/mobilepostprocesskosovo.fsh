...
float4 main(
    ...
    color.xyz *= 2.0;
#endif
    color.xyz = color.xyz * UserPSHParams[0].xyz;
    ...
    float glowIntensity = clamp(dot(glow.xyz-threshold.xyz,BW),0.0,1.0);
    ...
    // SceShacccg crashes with this one
    // color.xyz = lerp(glow.xyz, color.xyz, glow.w);
    color.xyz = lerp(color.xyz, glow.xyz, -glow.w);
#endif
    ...
    float cutoff = clamp((CustomPSHParams[0].x - intensity)*CustomPSHParams[0].z,0.0,1.0);
    ...
    float downFactor = clamp((intensity-CustomPSHParams[1].x)*CustomPSHParams[1].y,0.0,1.0);
    float upFactor = clamp((CustomPSHParams[1].z-intensity)*CustomPSHParams[1].w,0.0,1.0);
    float totalFactor = downFactor * upFactor;
    
    color.xyz = lerp(color.xyz,pencilEffectVec,clamp(totalFactor*CustomPSHParams[0].y,0.0,1.0));
    ...
    dist = clamp(dist * VignetteAndNoiseParams.x + VignetteAndNoiseParams.y,0.0,1.0);
    color.xyz *= dist * dist;
    ...
    gl_FragColor = float4(color.xyz,1.0);
